...
def logs():
  ...
  print((k+"["+p+"00"+k+"]"+p+" \x1b[0;33mLog Out"))
...
def publik():
	...
	try:
		...
		idt = input(k+"["+p+"?"+k+"]"+p+" ID Publik : \x1b[0;33m")
		try:
...
def follow():
	try:
		...
		logs()
	try:
		idt = input(k+"\n["+p+"?"+k+"]"+p+" Followers ID Publik : \x1b[0;33m")
		try:
			jok = requests.get("https://graph.facebook.com/"+idt+"?access_token="+toket)
			op = json.loads(jok.text)
...
def likers():
	try:
		...
		logs()
	try:
		idt = input(k+"\n["+p+"?"+k+"]"+p+" ID Postingan Publik : \x1b[0;33m")
		try:
			jok = requests.get("https://graph.facebook.com/"+idt+"?access_token="+toket)
			op = json.loads(jok.text)
			print((k+"["+p+"?"+k+"]"+p+" Nama : "+op["name"]))
...
def pilih_menu_user_agent():
    pmu = input("\n%s[%s•%s] %sChoose : \x1b[0;33m"%(k,p,k,p))
    if pmu in[""]:
        print((k+"\n["+p+"!"+k+"]"+p+" Fill In The Correct"))
    elif pmu in["1","01"]:
